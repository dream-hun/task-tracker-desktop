...
test('a line multiplies its quantity by its unit price', function (): void {
...
test('a fractional line total is rounded to the nearest cent', function (): void {
    $item = new DocumentItem(['description' => 'Line', 'quantity' => '0.33', 'unit_price' => '10']);

    expect($item->total_cents)->toBe(330);
});

test('the subtotal adds up every line', function (): void {
...
test('tax is charged over the discounted subtotal', function (): void {
...
test('a discount never pushes the total below nothing', function (): void {
